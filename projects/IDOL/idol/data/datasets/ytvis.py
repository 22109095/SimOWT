...
__all__ = ["load_ytvis_json", "register_ytvis_instances", "load_tao_json", "register_tao_instances"]
...
def _get_tao_instances_meta():
    # print('')
    try:
        json_file = 'datasets/tao/annotations/TAO_CATEGORIES.json'
        with open(json_file, 'r') as f:
            TAO_CATEGORIES = json.load(f)
    except:
        json_file = '/ssd1/wby_workspace/tao30/datasets/tao/annotations/TAO_CATEGORIES.json'
        with open(json_file, 'r') as f:
            TAO_CATEGORIES = json.load(f)
    thing_ids = [k["id"] for k in TAO_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in TAO_CATEGORIES if k["isthing"] == 1]
    assert len(thing_ids) == 1230, len(thing_ids)
    # Mapping from the incontiguous YTVIS category id to an id in [0, 39]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in TAO_CATEGORIES if k["isthing"] == 1]
    ret = {
...
def load_tao_json(json_file, image_root, dataset_name=None, extra_annotation_keys=None):  # [ATTN]
...
"""
    ...
    # 补充vids 里面的 length 和 file_names
    vid2imgs = {}
    for img_info in ytvis_api.dataset['images']:
        vid = img_info['video_id']
        file_name = img_info['file_name']
        # [TODO] 过滤漏掉的几张图
        file_path = os.path.join('datasets/tao/frames', file_name)
        if os.path.exists(file_path):
            vid2imgs.setdefault(vid, []).append(file_name)
        else:
            print('lost img = {}'.format(file_name))
    for vid_dict in vids:
        vid_dict['file_names'] = vid2imgs[vid_dict['id']]
...
def load_ytvis_json(json_file, image_root, dataset_name=None, extra_annotation_keys=None):  # [ATTN]
    from pycocotools.ytvos import YTVOS
...
def register_tao_instances(name, metadata, json_file, image_root):
    ...
    # since they might be useful in evaluation, visualization or logging
    MetadataCatalog.get(name).set(
        json_file=json_file, image_root=image_root, evaluator_type="ytvis", **metadata  # [TODO] evaluator_type
    )
