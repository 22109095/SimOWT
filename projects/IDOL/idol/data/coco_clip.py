...
class COCO_CLIP_DatasetMapper:
    ...
    def __init__(self, cfg, is_train=True):
        if cfg.INPUT.CROP.ENABLED and is_train:  # r50_coco_sequence = True
            self.crop_gen = [
        ...
        self.same_crop =  cfg.INPUT.PRETRAIN_SAME_CROP  # [ATTN] r50_coco_sequence = False
    ...
    def __call__(self, dataset_dict):  # only used for training
        ...
        try:
            if 'train2017/0' not in dataset_dict["file_name"]:
                dataset_dict["file_name"] = dataset_dict["file_name"].replace('coco/train2017', 'tao/frames/train')
                if "train/train" in dataset_dict["file_name"]:
                    dataset_dict["file_name"] = dataset_dict["file_name"].replace('/train/train/', '/train/')
        ...
        if not self.same_crop:
            ...
            key_image_shape = image_key.shape[:2]
            dataset_dict["image"] = []
            ...
            instances_key = utils.annotations_to_instances(annos_key, key_image_shape, mask_format="bitmask", transflow=transforms_key)
            ...
            instances_ref = utils.annotations_to_instances(annos_ref, ref_image_shape, mask_format="bitmask", transflow=transforms_ref)
