...
if __name__ == '__main__':
    ...
    for setting in config.keys():  # 所有config里面的参数都可以args输入
        if type(config[setting]) == list or type(config[setting]) == type(None):
            parser.add_argument("--" + setting, nargs='+')
        else:
            parser.add_argument("--" + setting)
    args = parser.parse_args().__dict__
    for setting in args.keys():  # 用args里的参数覆盖config里面的
    ...
    print('eval_config = {}'.format(eval_config))
    print('dataset_config = {}'.format(dataset_config))
    # Run code
